...
public class WarmLuckyOMilkItem extends LuckyOMilkItem {
    public WarmLuckyOMilkItem(Properties settings, Flavour flavour) {
        super(settings, flavour, "Warm Lucky O' Milk");
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem plain(Properties settings) {
        return new WarmLuckyOMilkItem(settings, Flavour.PLAIN);
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem strawberry(Properties settings) {
        return new WarmLuckyOMilkItem(settings, Flavour.STRAWBERRY);
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem choco(Properties settings) {
        return new WarmLuckyOMilkItem(settings, Flavour.CHOCO);
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem matcha(Properties settings) {
        return new WarmLuckyOMilkItem(settings, Flavour.MATCHA);
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem banana(Properties settings) {
        return new WarmLuckyOMilkItem(settings, Flavour.BANANA);
    }

    @Contract("_ -> new")
    public static @NotNull WarmLuckyOMilkItem luck(Properties settings) {
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level world, LivingEntity user) {
        if (user instanceof ServerPlayer player) {
            switch (this.flavour) {
                case PLAIN -> player.connection.send(new ClientboundSetActionBarTextPacket(
                  Component.translatableWithFallback("message.vanilla_backrooms.lucky_o_milk.plain_warm", "It tastes like soy milk")
                ));
        ...
        switch (this.flavour) {
            case PLAIN -> user.addEffect(new MobEffectInstance(MobEffects.SATURATION, 15 * 20, 2, false, false, true));
            case STRAWBERRY ->
            ...
            case LUCK -> user.addEffect(new MobEffectInstance(MobEffects.LUCK, 15 * 60 * 20, 4, false, false, true));
        }
